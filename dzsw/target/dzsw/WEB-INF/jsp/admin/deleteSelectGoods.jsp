...
<script type="text/javascript">
  		function confirmDelete(){
  			var n = document.deleteForm.goodids.length;
  			var count = 0;//统计没有选中的个数
  			for(var i = 0; i < n; i++){
  				if(!document.deleteForm.goodids[i].checked){
  					count++;
  				}else{
  					break;
  				}
  			...
  			if(n > 1){
  				if(count == n){
  	  			...
  	  			if(!document.deleteForm.goodids.checked){
  	  					alert("请选择被删除的商品！");
  	  					return false;
  	  			}
  			}	
  		...
  		function checkDel(goodid){
  			if(window.confirm("是否删除该商品？")){
  				window.location.href = "adminGoods/deleteAGoods?goodid="+goodid;
  			}
  		}
  </script>
</head>
			...
			<table border="1" bordercolor="PaleGreen">
				...
				<c:forEach items="${allGoods }" var="goods">
					<tr onmousemove="changeColor(this)" onmouseout="changeColor1(this)">
						<td><input type="checkbox" name="goodids"
							value="${goods.goodid }" />${goods.goodid }</td>
						<td>${goods.gName }</td>
						<td>${goods.grprice }</td>
						<td>${goods.gstore }</td>
						<td><a href="adminGoods/selectAGoods?goodid=${goods.goodid}"
							target="_blank">详情</a></td>
						<td><a href="javascript:checkDel('${goods.goodid }')">删除</a>
						</td>
					</tr>
				</c:forEach>
				<tr>
